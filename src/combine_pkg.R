...
file_list <- list.files(path = "data/oss/oss2/original/cran/logs/pkg/",
                        pattern = "^2019.*gz$",
                        full.names = TRUE)
...
for (f in file_list) {
  ...
  data_grp <- data[, .N, .(date, country, package)]
  #data <- fread(f)
  print(paste("Writing", f))
  dbWriteTable(con, "cran_logs_pkg_day", data_grp, row.names = F, append = TRUE)
}
